...
let client = require('../../js/script.js');
...
describe('Work with favourite city', () => {
    ...
    //     let cityName = 'qwerty';
    //     // const url = baseURL + '/favourites?q=' + cityName;
    //     // fetchMock.delete(url, delCityResponse);
    //     // let newCity = client.newCityLoaderInfo();
    //     chai.request(server).del('/favourites').send(cityName).end((err, res) => {
    //         expect(res).to.have.status(200);
    //         done();
    //     });
    //     // client.deleteCity(cityName);
    //     // console.log(document.getElementsByClassName('favoriteCities')[0]);
    //     // const currentCity = document.getElementsByClassName('favoriteCities')[0];
    //     // currentCity.innerHTML.should.be.eql(cityMiass);
    //     // done();
    // });
});